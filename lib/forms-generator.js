...
function makeSubmenu(ids, opts, menuId, id, url, attrs, entries, nocontent) {
  ...
  ids[result.id] = true;
  if(!nocontent) result.content = makeMenuTransaltion(opts, menuId, idObj);
...
function Menu(id, opts, attrs) {
  ...
  var ids = {};
  this.skel = makeSubmenu(ids, opts ? opts : {}, idObj, null, null, attrs, entrySpecs, true);
  this.expansions = {};
}
...
Menu.prototype.render = function(jade, i18n, insertObj) {
...
function makeFields(ids, opts, validators, values, formId, fieldSpecs) {
  ...
  for(var i=0; i<fieldSpecs.length; ++i) {
    ...
    if( !(data[1] === "div" && !data[0]) && checkFormID(ids, opts, formId, data[0])) {
