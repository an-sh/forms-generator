...
function makeFieldTransaltion(opts, formId, elemId, subId) {
  if(elemId.global) return __( elemId.toString() );
...
function attrsHelper(attrs, resultObj) {
  if(isArray(attrs)) {
    ...
    resultObj.secondaryAttrs = attrs[2] ? copyObjectProps(attrs[2]) : {};
    resultObj.tertiaryAttrs = attrs[3] ? copyObjectProps(attrs[3]) : {};
  } else {
    resultObj.attrs = attrs ? copyObjectProps(attrs) : {};
    resultObj.wrapperAttrs = {};
    resultObj.secondaryAttrs = {};
    resultObj.tertiaryAttrs = {};
  }
  delete resultObj.attrs.id;
  delete resultObj.wrapperAttrs.id;
  delete resultObj.secondaryAttrs.id;
  delete resultObj.tertiaryAttrs.id;
}
...
function entryIdHelper(ids, opts, values, resultObj, formId, fieldId, elemId,
  ...
  if(fieldType === "radio" || fieldType === "checkbox") {
    ...
    resultObj.secondaryAttrs["for"] = resultObj.id;
  }
}
...
function makeField(ids, opts, validators, values, formId, id, type, attrs, entrydata) {
  ...
  if(type !== "select" && type !== "textarea" && type !== "datalist" && type !== "button") {
  ...
  if(type === "checkbox" && !entrydata) {
    ...
    field.secondaryAttrs["for"] = formId + "-" + idStr;
    values[idStr] = [ idStr ];
  ...
  if(type === "reset" || type === "submit") {
    ...
    field.secondaryAttrs["for"] = fieldId;
  }
...
function Form(id, opts, attrs) {
  ...
  skel.secondaryAttrs.onload = skel.secondaryAttrs.onload ?
    skel.secondaryAttrs.onload : skel.id + "Onload()";
  skel.secondaryAttrs.name = skel.secondaryAttrs.name ?
    skel.secondaryAttrs.name : skel.attrs.target;
  skel.secondaryAttrs.width = 0;
  skel.secondaryAttrs.height = 0;
  skel.secondaryAttrs.tabindex = -1;
  skel.secondaryAttrs.hidden = true;
...
function makeCallbackClosure1(fn, data1) {
  return function(error) {
    fn(error, data1);
  };
}

function makeCallbackClosure2(fn, data1, data2) {
  return function(error) {
    fn(error, data1, data2);
  };
}
...
Form.prototype.runValidatator = function(field, data, i18n, cb) {
  ...
  if(fn) {
    fn(data, i18n, makeCallbackClosure1(cb, data));
  } else {
    cb(null, data);
  }
};
...
Form.prototype.runGlobalValidatator = function(fields, files, i18n, cb) {
  ...
  if(fn) {
    fn(fields, files, i18n, makeCallbackClosure2(cb, fields, files));
  } else {
...
Form.prototype.render = function(jade, i18n, insertObj) {
  var form = this.getContent(i18n);
  var includeFiles = Array.prototype.slice.call(arguments, 3);
  var opts = {
    'basedir' : "/",
    '__form' : form,
    '__insertObj' : insertObj ? insertObj : {}
  };
  var mixins = 'include ' + path.join(__dirname, "../jade/mixins.jade") + '\n';
  var code = makeJadeIncludes(includeFiles);
  return jade.render(mixins + code + '\n+Form(__form, __insertObj)', opts);
};
